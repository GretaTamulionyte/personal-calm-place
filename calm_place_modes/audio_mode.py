...
First select from the list of texts by typing the id number")
        ...
        while mode_running:
            ...
            print(f"You've selected the text called {self.texts_data[index_of_text]['title']}")
            if isinstance(self.texts_data[index_of_text]["text"],list):
                self.texts_data[index_of_text]["text"] = " ".join(self.texts_data[index_of_text]["text"])
            ...
            if voice_selection in available_voice_ids.keys():
                try:
                    response = client.text_to_speech.convert(
                        voice_id=available_voice_ids[voice_selection], # Adam pre-made voice
 ...
 of your choice and your custom relaxation tape will be all done.\n\n")
        ...
        while True:
            ...
            if sound_selection in available_sounds.keys():
                ...
                talking_audio = AudioSegment.from_file(f"{self.texts_data[index_of_text]['title']}.mp3")
                background_sounds = AudioSegment.from_file(f"{available_sounds[sound_selection]}")
                ...
                desired_ratio = 0.9  # Background should be 95% of main track's loudness
                adjustment_db = 20 * math.log10((talking_rms / background_rms) * desired_ratio)
                ...
                required_length = len(silence_before) + len(talking_audio) + len(silence_after)
                ...
                background_sounds = background_sounds.fade_in(fade_in_duration).fade_out(fade_out_duration)
                ...
                try:
                    audio_with_background.export(f"{self.texts_data[index_of_text]['title']}_finished.mp3", format="mp3")
                except Exception as e:
